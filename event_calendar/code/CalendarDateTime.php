...
class CalendarDateTime extends DataObject
{
	...
	public function getEventClass()
	{
		...
		$has_ones = eval("return $class::\$has_one;");
	...
	public function getAnnouncementPopup()
	{
		...
		$f->push(new SimpleTinyMCEField('Content',_t('CalendarDateTime.CONTENT','Content')));
		$f->push(new ImageField('Photo',_t('CalendarDateTime.PHOTO','Photo')));
	...
	private function initPopupFields() 
	{
	...
	private function initAnnouncementFields() 
	{
		$this->announcement_table_fields = array(
			...
			new CheckboxField('is_all_day',_t('CalendarEvent.ALLDAY','All Day')),
			new SimpleTinyMCEField('Content',_t('CalendarEvent.CONTENT','Content')),
			new ImageField('Photo',_t('CalendarDateTime.PHOTO','Photo')),
