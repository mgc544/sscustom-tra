...
/**
	...
	function extractVideoInfo($video) {
		...
		$urlObjs = $mediaentry->xpath('media:player');
		$data['VideoUrl'] = new ArrayData(array(
			'URL' => Convert::raw2xml((string)$urlObjs[0]['url']),
		));
		...
		$data['VideoURL'] = Convert::raw2xml((string)$urlObj[0]['url']);
		...
		$urlObj2 = $mediaentry->xpath('media:player[@url]');
		$data['PlayerVideoURL'] = Convert::raw2xml((string)$urlObj2[0]['url']);
